...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
    ...
    // if node is in between or equal to upper and lower, add to list
    if (n->rec <= upper && n->rec >= lower) {
        ListAppend(l, n->rec);
        doTreeSearchBetween(t, n->right, lower, upper, l);
        doTreeSearchBetween(t, n->left, lower, upper, l);
    }
    
    //if node is smaller than lower, go right and check
    if (n->rec < lower) {
        doTreeSearchBetween(t, n->right, lower, upper, l);
    }
    //if node is larger than upper, go left and check
    if (n->rec > upper) {
        doTreeSearchBetween(t, n->left, lower, upper, l);
    }
